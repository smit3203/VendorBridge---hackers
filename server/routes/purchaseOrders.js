...
router.get('/', authenticate, (req, res) => {
  ...
  if (search) {
    where.push('(po.po_number LIKE ? OR v.company_name LIKE ?)');
    params.push(`%${search}%`, `%${search}%`);
  }

  const total = db.prepare(`SELECT COUNT(*) as count FROM purchase_orders po JOIN vendors v ON po.vendor_id = v.id WHERE ${where.join(' AND ')}`).get(...params).count;
  ...
  purchaseOrders.forEach(po => {
    po.items = db.prepare('SELECT * FROM po_items WHERE po_id = ?').all(po.id);
  });
...
router.post('/', authenticate, authorize('admin', 'procurement_officer'), (req, res) => {
  ...
  // Check approval
  const approval = db.prepare("SELECT * FROM approvals WHERE rfq_id = ? AND action = 'approved'").get(rfq_id);
  if (!approval) {
    return res.status(400).json({ error: 'RFQ has not been approved yet' });
  }

  // Check no existing PO
  const existingPO = db.prepare('SELECT id FROM purchase_orders WHERE rfq_id = ?').get(rfq_id);
  if (existingPO) {
    return res.status(409).json({ error: 'Purchase order already exists for this RFQ' });
  }

  const quotation = db.prepare('SELECT * FROM quotations WHERE id = ?').get(quotation_id);
  ...
  const result = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'confirmed')
  `).run(poNumber, rfq_id, quotation_id, quotation.vendor_id, req.user.id, subtotal, rate, taxAmount, grandTotal);
  ...
  qItems.forEach(item => {
    insertPOItem.run(poId, item.product_name, item.description, item.quantity, item.unit, item.unit_price, item.total_price);
  });

  logActivity(req.user.id, req.user.name, 'po_created', 'purchase_order', poId, `Purchase Order ${poNumber} created`);
...
router.put('/:id', authenticate, authorize('admin', 'procurement_officer'), (req, res) => {
  ...
  if (status) {
    ...
    logActivity(req.user.id, req.user.name, 'po_status_updated', 'purchase_order', req.params.id, `PO ${po.po_number} status: ${status}`);
  }
